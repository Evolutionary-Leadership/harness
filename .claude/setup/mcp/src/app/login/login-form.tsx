...
export function LoginForm({ demoLogin }: { demoLogin: DemoLogin | null }) {
  ...
  async function signIn(withEmail: string, withPassword: string) {
    ...
    // An OAuth authorization flow can send a user here mid-flight, with its
    // signed query on the URL. The auth client attaches that query to the sign
    // in call, so the server answers with where to go next (the consent screen,
    // or straight back to the client). Honour it when present; a plain sign in
    // has no such continuation and lands on the notes list as before.
    const continueTo = (result.data as { url?: string } | null)?.url;
    router.replace(continueTo ?? "/notes");
  }
