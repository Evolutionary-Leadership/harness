...
An MCP tool is a fourth entry point at the boundary, and obeys the same rule:
it calls a service and never a repository. Add one with `/mcp-tool`; the tool
seam and the reason for it are in
...
migration. **`src/lib/db/schema/oauth.ts` is generated**, not hand-edited: run
`pnpm db:oauth-schema` first, then `pnpm db:generate`. A model a Better Auth
plugin declares must also be listed in the adapter's schema map in
`src/lib/auth.ts`, or it fails at runtime rather than at build time. Breaking changes use expand and contract. Every environment must be able
