...
| `oauth_client` | Registered OAuth clients | Generated; see [mcp-server.md](./mcp-server.md) |
| `oauth_resource` | Protected resources tokens may be issued for | Generated |
| `oauth_client_resource` | Which clients may reach which resource | Generated |
| `oauth_refresh_token` | Refresh tokens | Generated |
| `oauth_access_token` | Issued access tokens | Generated |
| `oauth_consent` | What a user granted a client | Generated |
| `oauth_client_assertion` | Replay protection for client assertions | Generated |
| `jwks` | Signing keys for issued tokens | Generated, from the `jwt()` plugin |
