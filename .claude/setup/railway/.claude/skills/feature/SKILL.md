...
.claude/scripts/set-feature-name.sh --no-push <slug> --preview=<yes|no>`: `yes` only for L
or when `/review` is the known exit, `no` otherwise (an S or M bound for `/to-preprod` gets
no preview; `/review` flips a `no` itself). It writes `.harness-feature` and commits;
checkpoint 1's push below carries the name, creates `feature/<slug>` and, on `yes`,
provisions a Railway preview environment under that name. It is idempotent. Do this in phase
0 even though no code exists yet, so provisioning runs in the background while you grill.
Then:
...
than aborting. On a `preview: yes` branch, show the Railway preview URL if one is already
published (`git show "origin/$FEATURE_BRANCH:.railway-url" 2>/dev/null || echo "URL not yet
available"`). Read `.harness/feature-context/$FEATURE_NAME.md` if the merge brought it in;
...
### The Railway preview at the exit

The exit decides what happens to the preview, and no session waits on a deploy: the
`Verify Railway deploy` workflow checks every preview after its provisioning run, `preprod`
after every merge and production after every release, and sets a `deploy/preview`,
`deploy/preprod` or `deploy/production` status on the commit that should be serving. On
`/to-preprod` or `/release`, print the URL if `.railway-url` exists on
`origin/$FEATURE_BRANCH` (`git show "origin/$FEATURE_BRANCH:.railway-url" 2>/dev/null`) and
move on: the merge path tears the environment down, `deploy/preprod` reports the merged
build, and `/release` verifies production in its own steps. On `/review` (a `preview: no`
branch is flipped there, and that push provisions), print the URL once it is published and
say that the pull request will show `deploy/preview` when the build is live. Only when the
user asks to wait, run the check in the session:

    bash .claude/scripts/get-railway-url.sh --wait
    bash .claude/scripts/verify-deploy.sh

and report its verdict: `deploy-verified:` (live, name the short sha); `deploy-equivalent:`
(current, and no deploy was due: this session changed no path Railway watches; never say
"verified" alone); `deploy-pending:` (still rolling; re-running re-checks). On the `/review`
path the preview lives until `feature-merge-cleanup.yml` tears it down after the merge.
