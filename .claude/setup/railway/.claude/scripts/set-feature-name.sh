...
# feature/<slug> and, with --preview=yes, provisions a Railway preview under
# that name. Run BEFORE the first push. Falls back to the random codename if
# never called.
#
...
if git push -u origin "$BRANCH" 2>&1; then
  echo "Feature name set: $SLUG  ->  feature/$SLUG (Railway preview: $PREVIEW)"
else
  (
