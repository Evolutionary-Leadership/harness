...
The harness scaffolds a minimal Node + Express "it works" app (`server.js`,
`package.json`, `.gitignore`) so the Railway pipeline has something to deploy
on the first push; a preview shows the branch, environment and git SHA. These
three files are **write-once**: `/harness-upgrade` never overwrites or
recreates them. Build your real app by editing `server.js` (keep the
`x-harness` response headers, which `verify-deploy.sh` reads), or delete all
three and update `railway.json`'s `startCommand` and `watchPatterns` for
another runtime.
...
The full lifecycle from idea to merged feature is automated. A Railway
preview environment is provisioned only when `.harness-feature` carries
`preview: yes` (an L change, or a run whose exit is `/review`); S and M
changes going to `/to-preprod` get none.

### Railway environments

Every environment (production, preprod, and each provisioned preview) has
its own isolated PostgreSQL instance, S3-compatible bucket and app service.
...
the preview URL. In every session:
...
- A preview's database starts **empty**; preprod and production only run
  pending migrations. Use expand-and-contract for breaking schema changes.
- Production is never seeded: honour `SEED_DATA=false` at the top of any
  seed script. `BETTER_AUTH_SECRET`, `SEED_DATA` and `SHOW_DEMO_LOGIN` are
  set per environment at provision time; read them, never set them by hand.
- **Preprod and every preview are publicly loginable** (a seeded demo
  account, `SHOW_DEMO_LOGIN=true`, a public URL). Preprod must never hold
  real data; production never gets that flag.
- Hooks never wait: the post-push hook prints `.railway-url` if it exists.
  Every deploy is checked in Actions (`verify-deploy.yml`): a
  `deploy/preview`, `deploy/preprod` or `deploy/production` commit status
  says whether the environment serves the commit it should. No session waits
  on a deploy unless the user asks it to.
...
names the branch (`set-feature-name.sh`, with `--preview=yes` for L or a
`/review` exit, which provisions Railway in the background), creates the
feature context and
...
on `/review` it reports the preview URL and verifies the deploy first.
...
Railway preview URL for live testing (the environment lives until the merge;
`/review` flips `preview:` to `yes` when it was `no`).
...
which deletes the Railway environment (Postgres and bucket included) and the
feature branch; `feature-branch-cleanup.yml` is the fallback for a branch
deleted by hand.
...
a colleague's half-finished work reaches production. The production Railway
...
go), `/hotfix`, `/rollback`, `/status` (with Railway preview URLs),
`/changelog`, `/deps`, `/continue`
