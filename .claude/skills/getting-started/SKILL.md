...
before the first push, so the feature branch gets a meaningful name instead of
the session's codename. `/feature #<issue>` picks up a `/brainstorm` idea
issue; `/continue` resumes an in-flight feature via its feature context.
