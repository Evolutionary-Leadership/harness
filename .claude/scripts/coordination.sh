...
# the candidate is always max(numbers on dev, numbers claimed) + 1, which
# keeps dev the source of truth and this branch a cache over it.
#
...
usage: coordination.sh <command> [args]
  ...
  next-adr <on-dev> <claimed>   Print max(on-dev, claimed) + 1, zero-padded
  ...
  adr-numbers-on-dev            Print the ADR numbers present on dev, one per
                                line. Falls back to the local working tree
                                when dev cannot be read.
USAGE
}
...
cmd_next_adr() {
  local on_dev="${1-}" claimed="${2-}" highest
  highest=$(
    { printf '%s\n%s\n' "$on_dev" "$claimed" | normalise; printf '0000\n'; } \
      | sort -n | tail -1
  )
  printf '%04d\n' "$((10#$highest + 1))"
}
...
cmd_adr_numbers_on_dev() {
  local listing
  git fetch --quiet "$REMOTE" dev 2>/dev/null || true
  listing=$(git ls-tree -r --name-only "$REMOTE/dev" docs/decisions/ 2>/dev/null || true)
  # No readable dev (offline, fresh clone, no remote): the local tree is the
...
case "${1-}" in
  ...
  list)                shift; cmd_list "$@" ;;
  adr-numbers-on-dev)  shift; cmd_adr_numbers_on_dev "$@" ;;
